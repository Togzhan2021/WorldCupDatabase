...
echo "$($PSQL "SELECT name FROM teams t FULL JOIN games g ON
g.winner_id = t.team_id OR g.opponent_id = t.team_id WHERE (round = 'Eighth-Final' and year = 2014) ORDER BY t.name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT(t.name) FROM games g LEFT JOIN teams t ON t.team_id = g.winner_id ORDER BY t.name")"
